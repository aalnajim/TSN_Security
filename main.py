...
from retriveResults import findLatestResults
...
def SWOTS_ASAP_WS(G, tempTSNFlow,scheduledFlowsSWOTS_ASAP_WS,CLength,time, FTT):
    if(CLength<tempTSNFlow.flowMaxDelay):
        return False
    publishTime = 15000 + 1000
    startTime = (time - FTT + publishTime)%CLength
    tempOperations = map(G,tempTSNFlow,startTime)       #line 652-654 added lately
...
def orderCollidedFlowsBasedOnTheEarliestArrivalTime(G, TSNFlow, collisionLocation, scheduledFlows, deletedScheduledFlows,
    ...
    if (len(scheduleItem) == 2):
        ...
        arrivalTime = operations.__getitem__(operationIndex - 1).cumulativeDelay + scheduleItem.__getitem__(2).__getitem__(
            queueIndex)
        unorderedListOfCollidedFlowsByEarliestArrivalTime.append((TSNFlow, arrivalTime, operations.__getitem__(operationIndex - 1).cumulativeDelay))
        for tempTSNFlow in collidedFlowList:
            scheduleItem = getSchedulingDetails(tempTSNFlow, scheduledFlows, deletedScheduledFlows)
            operations = map_ws(G, tempTSNFlow, scheduleItem.__getitem__(1), scheduleItem.__getitem__(2))
            operationIndex = getOperationIndexByOperationID(operations, operationID)
            ...
            unorderedListOfCollidedFlowsByEarliestArrivalTime.append((tempTSNFlow, arrivalTime, operations.__getitem__(operationIndex - 1).cumulativeDelay))

    orderedListOfCollidedFlowsByEarliestArrivalTime = sorted(unorderedListOfCollidedFlowsByEarliestArrivalTime,
                                                         key=lambda x: (x.__getitem__(1),x.__getitem__(1)), reverse=True)
    return orderedListOfCollidedFlowsByEarliestArrivalTime
...
def collisionResolveByDelayingCollidedFlows(G,scheduledFlows, deletedScheduledFlows,listofCollisions, collisionList,collisionPerEgressPortCounter, selectAlgorithm, CLength):
    listofCollisionsV2, collisionListV2 = copyCollisionlists(listofCollisions, collisionList)
    newCollisionPerEgressPortCounter = collisionPerEgressPortCounter
    listOfDropedFlows = []
    if(selectAlgorithm == 0):
    ...
    for listItem in listofCollisions:
        ...
        for collisionLocation in listItem.__getitem__(4):
            ...
            listOfCollidedTSNFlows = [tempItem.__getitem__(0) for tempItem in orderedListOfStatisticsPerFlows]
            queuingDelayFlag = False
            # firstFlowFlag = True
            # initialDelay = 0
            for i in range(startCondition,-1,-1):
                ...
                for j in range(startCondition, i, -1):
                    ...
                    if (((firstScheduledFlowFinishTime >= secondScheduledFlowFinishTime) and (
                        ...
                        if(((firstScheduledFlowFinishTime + causedDelay - initialTime) >= CLength) or ((firstScheduledFlowOperations.__getitem__(-1).cumulativeDelay + causedDelay - firstScheduledFlowStartTime) > firstScheduledFlow.flowMaxDelay)):
                            flag = True
                            ...
                            queuingDelayFlag = adjustQueuingDelay(firstScheduledFlowQueuingDelays,firstScheduledFlowQueuingIndex,causedDelay,queuingDelayFlag)
                        ...
                        queuingDelayFlag = adjustQueuingDelay(firstScheduledFlowQueuingDelays,firstScheduledFlowQueuingIndex,(-1 * adjustedQueuingDelay),queuingDelayFlag)
                ...
                while(not exitFlag):
                    ...
                    for scheduledItem2 in newScheduledFlows:
                        if((scheduledItem2.__getitem__(0) in listOfDropedFlows) or (scheduledItem2.__getitem__(0) in listOfCollidedTSNFlows and listOfCollidedTSNFlows.index(scheduledItem2.__getitem__(0)) <= i)):
                            continue
                        ######################################################################################
                    ...
                    for deletedItem in newDeletedScheduledFlows:
                        if ((deletedItem.__getitem__(0) in listOfDropedFlows) or (deletedItem.__getitem__(0) in listOfCollidedTSNFlows and listOfCollidedTSNFlows.index(deletedItem.__getitem__(0)) < i)):
                            continue
                    ...
                    if (isAdjusted):
                        ...
                        if(dict[firstScheduledFlow.id] == firstScheduledFLowCollidedOperationIndex):
                            if(dict[firstScheduledFlow.id] < len(firstScheduledFlowOperations) - 2):
                                ...
                                # Make sure that we did not cause any new collision#
                                ############################################################################################################################
                                if(queuingDelayFlag):
                                    for tempOperation in firstScheduledFlowOperations[firstScheduledFLowCollidedOperationIndex+2::2]:
                                        forLoopBreakFlag = False
                                        tempCollisionLocation = convertOperationIDtoEdge(tempOperation.id)
                                        tempCollisionList = computeListOfCollisionsForAFlowInEgressPort(collisionListV2,tempCollisionLocation,firstScheduledFlow)
                                        if(len(tempCollisionList)>0):
                                            break
                                        else:
                                            innerWhileLoopExitFlag = False
                                            while (not innerWhileLoopExitFlag):
                                                isAdjusted = False
                                                firstScheduledFlowOperations = map_ws(G, firstScheduledFlow,firstScheduledFlowStartTime,firstScheduledFlowQueuingDelays)
                                                firstScheduledFLowCollidedOperationIndex = getOperationIndexByOperationID(firstScheduledFlowOperations, tempOperation.id)
                                                firstScheduledFlowQueuingIndex = int((firstScheduledFLowCollidedOperationIndex / 2) - 1)
                                                for scheduledItem2 in newScheduledFlows:
                                                    if ((scheduledItem2.__getitem__(0) in listOfDropedFlows) or (scheduledItem2.__getitem__(0) in listOfCollidedTSNFlows and listOfCollidedTSNFlows.index(scheduledItem2.__getitem__(0)) <= i)):
                                                        continue
                                                    ...
                                                    secondScheduledFlowQueuingDelays = scheduledItem2.__getitem__(2)
                                                    secondScheduledFlowOperations = map_ws(G, secondScheduledFlow,secondScheduledFlowStartTime,secondScheduledFlowQueuingDelays)
                                                    secondScheduledFLowCollidedOperationIndex = getOperationIndexByOperationID(secondScheduledFlowOperations, tempOperation.id)
                                                    if (secondScheduledFLowCollidedOperationIndex != -1):
                                                        secondScheduledFLowCollidedOperationIndex = getOperationIndexByOperationID(secondScheduledFlowOperations, tempOperation.id)
                                                        secondScheduledFlowQueuingIndex = int((secondScheduledFLowCollidedOperationIndex / 2) - 1)
                                                        firstScheduledFlowArrivalTime = firstScheduledFlowOperations.__getitem__(firstScheduledFLowCollidedOperationIndex - 1).cumulativeDelay + firstScheduledFlowQueuingDelays.__getitem__(firstScheduledFlowQueuingIndex)
                                                        ...
                                                        # This if statement to ensure that there is a collision between these two flows#
                                                        if (((firstScheduledFlowFinishTime >= secondScheduledFlowFinishTime) and (firstScheduledFlowArrivalTime < secondScheduledFlowFinishTime)) or ((firstScheduledFlowFinishTime > secondScheduledFlowArrivalTime) and (firstScheduledFlowArrivalTime <= secondScheduledFlowArrivalTime)) or ((firstScheduledFlowFinishTime <= secondScheduledFlowFinishTime) and (firstScheduledFlowArrivalTime >= secondScheduledFlowArrivalTime))):
                                                            causedDelay = secondScheduledFlowFinishTime - firstScheduledFlowArrivalTime
                                                            isAdjusted = True
                                                            if (((firstScheduledFlowFinishTime + causedDelay - firstScheduledFlowOperations.__getitem__(firstScheduledFLowCollidedOperationIndex - 1).cumulativeDelay) >= CLength) or ((firstScheduledFlowOperations.__getitem__(-1).cumulativeDelay + causedDelay - firstScheduledFlowStartTime) > firstScheduledFlow.flowMaxDelay)):
                                                                innerWhileLoopExitFlag = True
    ...
    return listofCollisionsV2, collisionListV2, newCollisionPerEgressPortCounter, listOfDropedFlows, newScheduledFlows, newDeletedScheduledFlows
...
def collisionResolveByDelaying(G,scheduledFlows, deletedScheduledFlows,listofCollisions, collisionList,collisionPerEgressPortCounter, selectAlgorithm, CLength):
    ...
    listofCollisionsV2, collisionListV2, newCollisionPerEgressPortCounter, listOfDropedFlows, newScheduledFlows, newDeletedScheduledFlows = collisionResolveByDelayingCollidedFlows(
...
def collisionResolve(G,scheduledFlows, deletedScheduledFlows,listofCollisions, collisionList, collisionPerEgressPortCounter, resolveMethod, selectAlgorithm, CLength):
    ...
    if (resolveMethod == 0):
        ...
        return listofCollisionsV2, collisionListV2, newCollisionPerEgressPortCounter, listOfDropedFlows, scheduledFlows, deletedScheduledFlows
    else:
        listofCollisionsV2, collisionListV2, newCollisionPerEgressPortCounter, listOfDropedFlows, newScheduledFlows, newDeletedScheduledFlows = collisionResolveByDelaying(G,scheduledFlows, deletedScheduledFlows,listofCollisions, collisionList,collisionPerEgressPortCounter, selectAlgorithm, CLength)
        return listofCollisionsV2, collisionListV2, newCollisionPerEgressPortCounter, listOfDropedFlows, newScheduledFlows, newDeletedScheduledFlows
...
def insertAttack(G, hostsList , firstKthPaths, timeSlotsAmount, nbOfTSNFlows, pFlow, TSNCountWeight, bandwidthWeight, hopCountWeight,
    ...
    if(routedCounter == 0):
        routedCounter = 0.0000000000001
    return (nbOfTSNFlows,totalRoutedCounter,routedCounter,scheduledCounter,(scheduledCounter/nbOfTSNFlows)*100)
...
def deleteAttack(G, hostsList , firstKthPaths, timeSlotsAmount, nbOfTSNFlows, pFlow, TSNCountWeight, bandwidthWeight, hopCountWeight,
                 typeOfSecurityAttack, attackRate, attackStartTime, typeofSchedulingAlgorithm,collisionResolveMethod,collisionResolveSelectAlgorithm):
    CLength = 0  # the schedule cycle length
    timeSlotLength = 0  # the length of each time slot
    ...
    if(len(deletedScheduledFlows) != 0):
        ...
        collisionList = []
        collisionPerEgressPortCounter = 0
    ...
    ########################################## START COMMENT 10/18/2021 ##########################################
    if((collisionResolveMethod != -1 and collisionResolveSelectAlgorithm !=-1)):
        resolveMethod = collisionResolveMethod                        # 0 for dropping and 1 for delaying
        selectAlgorithm = collisionResolveSelectAlgorithm             # 0,1,2 or 3 check the method body to understand the meaning of this algorithms
    ...
    #     print("=======================================================================")
    #     print()
    #     print(">>>> Scheduled Flows Version I <<<<")
    #     print()
    #     for scheduledItem in scheduledFlows:
    #         TF = scheduledItem.__getitem__(0)
    #         startTime = scheduledItem.__getitem__(1)
    #         if(len(scheduledItem)>2):
    #             queuingDelays = scheduledItem.__getitem__(2)
    #             print('Flow# {} starts transmission at {} and its queuing delays are as follow: {}'.format(TF.id,startTime,queuingDelays))
    #             operations = map_ws(G,TF,startTime,queuingDelays)
    #         else:
    #             print('Flow# {} starts transmission at {}'.format(TF.id, startTime))
    #             operations = map(G, TF, startTime)
    #         print("       Its path is as follow: (",end="")
    #         for i in range(len(scheduledItem.__getitem__(0).path.nodes)):
    #             if(i ==0):
    #                 print(scheduledItem.__getitem__(0).path.nodes.__getitem__(i).id,end=", ")
    #             elif(i == len(scheduledItem.__getitem__(0).path.nodes)-1):
    #                 print("{})".format(scheduledItem.__getitem__(0).path.nodes.__getitem__(i).id))
    #             else:
    #                 print(scheduledItem.__getitem__(0).path.nodes.__getitem__(i), end=", ")
    #
    #         if (len(scheduledItem) == 2):
    #             print(O+"       Its deadline {} and startTime {} has the following operations: \n       [".format(TF.flowMaxDelay,startTime),end="")
    #             index = 0
    #             for operation in operations:
    #                 if(index != len(operations)-1):
    #                     print("{}:{}], [".format(operation.id,operation.cumulativeDelay),end="")
    ...
    #     print("=======================================================================")
    #     print()
    #     print(">>>> Scheduled Deleted Flows Version I <<<<")
    #     print()
    #     for scheduledItem in deletedScheduledFlows:
    #         TF = scheduledItem.__getitem__(0)
    #         startTime = scheduledItem.__getitem__(1)
    #         if (len(scheduledItem) > 2):
    #             queuingDelays = scheduledItem.__getitem__(2)
    #             print('Flow# {} starts transmission at {} and its queuing delays are as follow: {}'.format(TF.id, startTime,
    #                                                                                                        queuingDelays))
    #             operations = map_ws(G, TF, startTime, queuingDelays)
    #         else:
    #             print('Flow# {} starts transmission at {}'.format(TF.id, startTime))
    ...
    #         if (len(scheduledItem) == 2):
    #             print(
    #                 B + "       Its deadline {} and startTime {} has the following operations: \n       [".format(TF.flowMaxDelay,
    #                                                                                                 startTime), end="")
    #             index = 0
    #             for operation in operations:
    #                 if (index != len(operations) - 1):
    ...
    #                         print("{}:{}], [".format(operation.id, operation.cumulativeDelay), end="")
    #                     elif (index % 2 == 0):
    #                         print("{}:{}:QD={}], [".format(operation.id, operation.cumulativeDelay,
    #                                                        queuingDelays.__getitem__(int((index / 2) - 1))), end="")
    #                 else:
    #                     print("{}:{}]".format(operation.id, operation.cumulativeDelay))
    #
    #                 index = index + 1
    ...
    # #################################################################################################
        ...
        listofCollisionsV2, collisionListV2, newCollisionPerEgressPortCounter, listOfDropedFlows, newScheduledFlows, newDeletedScheduledFlows = collisionResolve(G,scheduledFlows, deletedScheduledFlows,
    ...
    #     print("=======================================================================")
    #     print()
    #     print(">>>> Scheduled Flows Version II <<<<")
    #     print()
    ...
    #         if (len(scheduledItem) > 2):
    #             queuingDelays = scheduledItem.__getitem__(2)
    #             print('Flow# {} starts transmission at {} and its queuing delays are as follow: {}'.format(TF.id,
    #                                                                                                        startTime,
    #                                                                                                        queuingDelays))
    #             operations = map_ws(G, TF, startTime, queuingDelays)
    #         else:
    ...
    #         if (len(scheduledItem) == 2):
    #             print(
    #                 O + "       Its deadline {} and startTime {} has the following operations: \n       [".format(TF.flowMaxDelay,
    #                                                                                                               startTime),
    #                 end="")
    #             index = 0
    #             for operation in operations:
    ...
    #     print("=======================================================================")
    #     print()
    #     print(">>>> Scheduled Deleted Flows Version II <<<<")
    #     print()
    #     for scheduledItem in newDeletedScheduledFlows:
    #         TF = scheduledItem.__getitem__(0)
    #         startTime = scheduledItem.__getitem__(1)
    #         if (len(scheduledItem) > 2):
    #             queuingDelays = scheduledItem.__getitem__(2)
...
###########
    ...
    if(collisionResolveSelectAlgorithm == -1 or collisionResolveMethod == -1):
        return (nbOfTSNFlows,routedCounter,scheduledCounter,totalscheduledCounter,totalscheduledCounter-scheduledCounter,listofCollisions,collisionList,collisionPerEgressPortCounter)
    return (nbOfTSNFlows,routedCounter,scheduledCounter,totalscheduledCounter,totalscheduledCounter-scheduledCounter,listofCollisions,collisionList,collisionPerEgressPortCounter,listOfDropedFlows)
...
def testSecurityImpact(typeOfSecurityAttack, attackRate, attackStartTime, typeofSchedulingAlgorithm,collisionResolveMethod,collisionResolveSelectAlgorithm,listOfResults):
        ...
        if(typeOfSecurityAttack in [0,1]):
            listOfResults.append(insertAttack(G, hostsList , firstKthPaths, timeSlotsAmount, nbOfTSNFlows, pFlow, TSNCountWeight, bandwidthWeight, hopCountWeight,
                         typeOfSecurityAttack, attackRate, attackStartTime, typeofSchedulingAlgorithm))
        else:
            listOfResults.append(deleteAttack(G, hostsList , firstKthPaths, timeSlotsAmount, nbOfTSNFlows, pFlow, TSNCountWeight, bandwidthWeight, hopCountWeight,
                         typeOfSecurityAttack, attackRate, attackStartTime, typeofSchedulingAlgorithm,collisionResolveMethod,collisionResolveSelectAlgorithm))
...
def simulateCase3(typeOfSecurityAttack, intensivityOfTheAttack, attackStartTime, typeofSchedulingAlgorithm,simulatorAmountOfRuns, schedulingAlgorithms, attackTypes,resolveMethods,selectAlgorithms,collisionResolveMethod,collisionResolveSelectAlgorithm, now, returnedValue = None):
    try:
        listOfValues = attackStartTime.replace(" ", "").split(",")
        if ((len(listOfValues) != 3) or (float(listOfValues.__getitem__(0)) < 0 or float(
                listOfValues.__getitem__(0)) > 1) or
                (float(listOfValues.__getitem__(1)) < 0 or float(
                    listOfValues.__getitem__(1)) > 1) or (
                        float(listOfValues.__getitem__(2)) <= 0 or float(
                    listOfValues.__getitem__(2)) > 1) or (
                        float(listOfValues.__getitem__(0)) >= float(
                    listOfValues.__getitem__(1)))):
            raise Exception()
        startValue = float(listOfValues.__getitem__(0))
        endValue = float(listOfValues.__getitem__(1))
        incrementValue = float(listOfValues.__getitem__(2))
    except:
        errorMessage = "Enter a correct value for the attack start time parameter from 0 to 1 or in the form 'startVelue,endValue,incrementValue' to select multiple values starting from startVelue to endValue with an increment of incrementValue"
    ...
    listOfMaxs = []
    listOfMeans = []
    if (typeOfSecurityAttack in [2,3]):
        listOfMeans1 = []
        listOfMeans2 = []
        if(collisionResolveMethod != -1 and collisionResolveSelectAlgorithm != -1):
            listOfMeans3 = []
    ...
    listOfStDevs = []
    listOfVariances = []
    attackStartTimeParameterRange = "startTime='{},{},{}'".format(startValue,endValue,incrementValue)
    preText = O + "\t\t\t# Attack Start Time: "
    if (typeOfSecurityAttack in [0, 1]):
        while (startValue <= currentValue <= endValue):
            print(preText + str(currentValue) + W)
            resultsStatistics = simulateCase2(typeOfSecurityAttack, intensivityOfTheAttack, currentValue,typeofSchedulingAlgorithm, simulatorAmountOfRuns, schedulingAlgorithms,attackTypes, now)  # a tupple of (min,max,mean,median,mode,stdev,variance)
            listOfStartTimes.extend([currentValue for x in range(len(resultsStatistics.__getitem__(0)))])
            listOfAttackRates.extend(resultsStatistics.__getitem__(0))
            listOfMins.extend(resultsStatistics.__getitem__(1))
            listOfMaxs.extend(resultsStatistics.__getitem__(2))
            listOfMeans.extend(resultsStatistics.__getitem__(3))
            listOfMedians.extend(resultsStatistics.__getitem__(4))
            listOfModes.extend(resultsStatistics.__getitem__(5))
            listOfStDevs.extend(resultsStatistics.__getitem__(6))
            listOfVariances.extend(resultsStatistics.__getitem__(7))
            currentValue = round(currentValue + incrementValue, 10)
            if ((currentValue > endValue) and (round(currentValue - incrementValue,10) < endValue)):
                currentValue = endValue
        intensivityOfTheAttackStartValue, intensivityOfTheAttackEndValue, intensivityOfTheAttackEncrementValue = intensivityOfTheAttack.split(",")
        intensivityOfTheAttackParameterRange = "attackRate='{},{},{}'".format(intensivityOfTheAttackStartValue,intensivityOfTheAttackEndValue,intensivityOfTheAttackEncrementValue)
        simulationParameters = (simulatorAmountOfRuns, schedulingAlgorithms.__getitem__(typeofSchedulingAlgorithm),attackTypes.__getitem__(typeOfSecurityAttack), intensivityOfTheAttackParameterRange, attackStartTimeParameterRange)
        summaryOfResults = (listOfStartTimes,listOfAttackRates, listOfMins, listOfMaxs, listOfMeans, listOfMedians, listOfModes, listOfStDevs, listOfVariances)
        saveResults.writeInsertResultsToFileCase2(summaryOfResults, simulationParameters, now)

    else:
        while (startValue <= currentValue <= endValue):
            print(preText + str(currentValue) + W)
            resultsStatistics = simulateCase2(typeOfSecurityAttack, intensivityOfTheAttack, currentValue,typeofSchedulingAlgorithm, simulatorAmountOfRuns, schedulingAlgorithms,attackTypes,resolveMethods,selectAlgorithms,collisionResolveMethod,collisionResolveSelectAlgorithm, now)  # a tupple of (min,max,mean,median,mode,stdev,variance)
            listOfStartTimes.extend([currentValue for x in range(len(resultsStatistics.__getitem__(0)))])
            listOfAttackRates.extend(resultsStatistics.__getitem__(0))
            listOfMins.extend(resultsStatistics.__getitem__(1))
            listOfMaxs.extend(resultsStatistics.__getitem__(2))
            listOfMeans.extend(resultsStatistics.__getitem__(3))
            listOfMeans1.extend(resultsStatistics.__getitem__(4))
            listOfMeans2.extend(resultsStatistics.__getitem__(5))
            if (collisionResolveMethod != -1 and collisionResolveSelectAlgorithm != -1):
                listOfMeans3.extend(resultsStatistics.__getitem__(6))
                listOfMedians.extend(resultsStatistics.__getitem__(7))
                listOfModes.extend(resultsStatistics.__getitem__(8))
                listOfStDevs.extend(resultsStatistics.__getitem__(9))
                listOfVariances.extend(resultsStatistics.__getitem__(10))
            else:
                listOfMedians.extend(resultsStatistics.__getitem__(6))
                listOfModes.extend(resultsStatistics.__getitem__(7))
                listOfStDevs.extend(resultsStatistics.__getitem__(8))
                listOfVariances.extend(resultsStatistics.__getitem__(9))
            currentValue = round(currentValue + incrementValue, 10)
            if ((currentValue > endValue) and (round(currentValue - incrementValue,10) < endValue)):
                currentValue = endValue
        intensivityOfTheAttackStartValue, intensivityOfTheAttackEndValue, intensivityOfTheAttackEncrementValue = intensivityOfTheAttack.split(",")
        intensivityOfTheAttackParameterRange = "attackRate='{},{},{}'".format(intensivityOfTheAttackStartValue,intensivityOfTheAttackEndValue,intensivityOfTheAttackEncrementValue)
        if (collisionResolveMethod != -1 and collisionResolveSelectAlgorithm != -1):
            simulationParameters = (simulatorAmountOfRuns, schedulingAlgorithms.__getitem__(typeofSchedulingAlgorithm),attackTypes.__getitem__(typeOfSecurityAttack), intensivityOfTheAttackParameterRange,attackStartTimeParameterRange,resolveMethods.__getitem__(collisionResolveMethod),selectAlgorithms.__getitem__(collisionResolveSelectAlgorithm))
            summaryOfResults = (listOfStartTimes, listOfAttackRates, listOfMins, listOfMaxs, listOfMeans, listOfMeans1, listOfMeans2, listOfMeans3, listOfMedians, listOfModes,listOfStDevs, listOfVariances)
        else:
            simulationParameters = (simulatorAmountOfRuns, schedulingAlgorithms.__getitem__(typeofSchedulingAlgorithm),attackTypes.__getitem__(typeOfSecurityAttack), intensivityOfTheAttackParameterRange, attackStartTimeParameterRange)
            summaryOfResults = (listOfStartTimes,listOfAttackRates, listOfMins, listOfMaxs, listOfMeans, listOfMeans1, listOfMeans2, listOfMedians, listOfModes, listOfStDevs, listOfVariances)
        saveResults.writeDeleteResultsToFileCase2(summaryOfResults, simulationParameters, now)
        
    if(returnedValue != None):
        returnedValue.append((schedulingAlgorithms.__getitem__(typeofSchedulingAlgorithm),resultsStatistics))
    return summaryOfResults




def simulateCase2(typeOfSecurityAttack, intensivityOfTheAttack, attackStartTime, typeofSchedulingAlgorithm,simulatorAmountOfRuns, schedulingAlgorithms, attackTypes,resolveMethods,selectAlgorithms,collisionResolveMethod,collisionResolveSelectAlgorithm, now, returnedValue = None):
    # all parameters are represented by a single value (no ranges) except for intensivityOfTheAttack or attackStartTime
    isAttackRate = (False if isinstance(intensivityOfTheAttack, float) else True)


    try:
        if(isAttackRate):
            listOfValues = intensivityOfTheAttack.replace(" ", "").split(",")
        else:
            listOfValues = attackStartTime.replace(" ", "").split(",")
        if ((len(listOfValues) != 3) or (float(listOfValues.__getitem__(0)) < 0 or float(
                listOfValues.__getitem__(0)) > 1) or
        ...
        errorMessage = "Enter a correct value for {} parameter from 0 to 1 or in the form 'startVelue,endValue,incrementValue' to select multiple values starting from startVelue to endValue with an increment of incrementValue".format(("the intensivity of the attack" if isAttackRate else "the attack start time"))
        raise ValueError(errorMessage)
    currentValue = startValue
    listOfValues = []
    listOfMins = []
    ...
    listOfStDevs = []
    listOfVariances = []
    parameterRange = ("attackRate='{},{},{}'".format(startValue,endValue,incrementValue) if isAttackRate else "startTime='{},{},{}'".format(startValue,endValue,incrementValue))
    preText = (B + "\t\t\t\t# Attack Rate: " if isAttackRate else O + "\t\t\t# Attack Start Time: ")
    if (typeOfSecurityAttack in [0, 1]):
        while (startValue <= currentValue <= endValue):
            print(preText + str(currentValue) + W)
            if(isAttackRate):
                resultsStatistics = simulateCase1(typeOfSecurityAttack, currentValue, attackStartTime,typeofSchedulingAlgorithm, simulatorAmountOfRuns, schedulingAlgorithms,attackTypes, now)  # a tupple of (min,max,mean,median,mode,stdev,variance)
            else:
                resultsStatistics = simulateCase1(typeOfSecurityAttack, intensivityOfTheAttack, currentValue,typeofSchedulingAlgorithm, simulatorAmountOfRuns, schedulingAlgorithms,attackTypes, now)  # a tupple of (min,max,mean,median,mode,stdev,variance)
            listOfValues.append(currentValue)
            listOfMins.append(resultsStatistics.__getitem__(0))
            listOfMaxs.append(resultsStatistics.__getitem__(1))
            listOfMeans.append(resultsStatistics.__getitem__(2))
            listOfMedians.append(resultsStatistics.__getitem__(3))
            listOfModes.append(resultsStatistics.__getitem__(4))
            listOfStDevs.append(resultsStatistics.__getitem__(5))
            listOfVariances.append(resultsStatistics.__getitem__(6))
            currentValue = round(currentValue + incrementValue, 10)
            if ((currentValue > endValue) and (round(currentValue - incrementValue,10) < endValue)):
                currentValue = endValue
        if(isAttackRate):
            simulationParameters = (simulatorAmountOfRuns, schedulingAlgorithms.__getitem__(typeofSchedulingAlgorithm),
                                attackTypes.__getitem__(typeOfSecurityAttack), parameterRange, attackStartTime)
        else:
            simulationParameters = (simulatorAmountOfRuns, schedulingAlgorithms.__getitem__(typeofSchedulingAlgorithm),
                                    attackTypes.__getitem__(typeOfSecurityAttack), intensivityOfTheAttack, parameterRange)
        summaryOfResults = (listOfValues, listOfMins, listOfMaxs, listOfMeans, listOfMedians, listOfModes, listOfStDevs, listOfVariances)
        saveResults.writeInsertResultsToFileCase2(summaryOfResults,simulationParameters,now)
        ...
        while (startValue <= currentValue <= endValue):
            print(preText + str(currentValue) + W)
            if(isAttackRate):
                resultsStatistics = simulateCase1(typeOfSecurityAttack, currentValue, attackStartTime,typeofSchedulingAlgorithm, simulatorAmountOfRuns, schedulingAlgorithms,attackTypes, resolveMethods,selectAlgorithms,collisionResolveMethod,collisionResolveSelectAlgorithm, now)  # a tupple of (min,max,mean,median,mode,stdev,variance)
            else:
                resultsStatistics = simulateCase1(typeOfSecurityAttack, intensivityOfTheAttack, currentValue,typeofSchedulingAlgorithm, simulatorAmountOfRuns, schedulingAlgorithms,attackTypes, resolveMethods,selectAlgorithms,collisionResolveMethod,collisionResolveSelectAlgorithm, now)  # a tupple of (min,max,mean,median,mode,stdev,variance)
            listOfValues.append(currentValue)
            listOfMins.append(resultsStatistics.__getitem__(0))
            listOfMaxs.append(resultsStatistics.__getitem__(1))
            listOfMeans.append(resultsStatistics.__getitem__(2))
            listOfMeans1.append(resultsStatistics.__getitem__(3))
            listOfMeans2.append(resultsStatistics.__getitem__(4))
            if(collisionResolveMethod != -1 and collisionResolveSelectAlgorithm != -1):
                listOfMeans3.append(resultsStatistics.__getitem__(5))
                listOfMedians.append(resultsStatistics.__getitem__(6))
                listOfModes.append(resultsStatistics.__getitem__(7))
                listOfStDevs.append(resultsStatistics.__getitem__(8))
                listOfVariances.append(resultsStatistics.__getitem__(9))
            else:
                listOfMedians.append(resultsStatistics.__getitem__(5))
                listOfModes.append(resultsStatistics.__getitem__(6))
                listOfStDevs.append(resultsStatistics.__getitem__(7))
                listOfVariances.append(resultsStatistics.__getitem__(8))
            currentValue = round(currentValue + incrementValue, 10)
            if ((currentValue > endValue) and (round(currentValue - incrementValue,10) < endValue)):
                currentValue = endValue
        ...
        if (collisionResolveMethod != -1 and collisionResolveSelectAlgorithm != -1):
            if(isAttackRate):
                simulationParameters = (simulatorAmountOfRuns, schedulingAlgorithms.__getitem__(typeofSchedulingAlgorithm),attackTypes.__getitem__(typeOfSecurityAttack), parameterRange, attackStartTime,resolveMethods.__getitem__(collisionResolveMethod),selectAlgorithms.__getitem__(collisionResolveSelectAlgorithm))
            else:
                simulationParameters = (simulatorAmountOfRuns, schedulingAlgorithms.__getitem__(typeofSchedulingAlgorithm),attackTypes.__getitem__(typeOfSecurityAttack), intensivityOfTheAttack, parameterRange,resolveMethods.__getitem__(collisionResolveMethod),selectAlgorithms.__getitem__(collisionResolveSelectAlgorithm))

            summaryOfResults = (listOfValues, listOfMins, listOfMaxs, listOfMeans, listOfMeans1, listOfMeans2, listOfMeans3, listOfMedians, listOfModes, listOfStDevs, listOfVariances)


        else:
            if (isAttackRate):
                simulationParameters = (simulatorAmountOfRuns, schedulingAlgorithms.__getitem__(typeofSchedulingAlgorithm),attackTypes.__getitem__(typeOfSecurityAttack), parameterRange, attackStartTime)
            else:
                simulationParameters = (simulatorAmountOfRuns, schedulingAlgorithms.__getitem__(typeofSchedulingAlgorithm),attackTypes.__getitem__(typeOfSecurityAttack), intensivityOfTheAttack, parameterRange)

            summaryOfResults = (listOfValues, listOfMins, listOfMaxs, listOfMeans, listOfMeans1, listOfMeans2, listOfMedians,listOfModes, listOfStDevs, listOfVariances)
...
def simulateCase1(typeOfSecurityAttack, intensivityOfTheAttack, attackStartTime, typeofSchedulingAlgorithm,simulatorAmountOfRuns, schedulingAlgorithms, attackTypes,resolveMethods,selectAlgorithms,collisionResolveMethod,collisionResolveSelectAlgorithm, now, returnedValue = None):
    # all parameters are represented by a single value (no ranges)
    # listOfResults = []
    # for i in range(simulatorAmountOfRuns):
    #     print(G + "\t\t\t[+] {}-run# {}".format(schedulingAlgorithms.__getitem__(typeofSchedulingAlgorithm),i+1) + W)
    ...
    listOfResults = manager.list()

    for i in range(simulatorAmountOfRuns):
        print(G + "\t\t\t\t\t[+] {}-run# {}".format(schedulingAlgorithms.__getitem__(typeofSchedulingAlgorithm),i+1) + W)
        p = multiprocessing.Process(target=testSecurityImpact, args=(typeOfSecurityAttack, intensivityOfTheAttack, attackStartTime,typeofSchedulingAlgorithm,collisionResolveMethod,collisionResolveSelectAlgorithm,listOfResults,))
        processes.append(p)
        p.start()
    ...
    if(typeOfSecurityAttack in [0,1]):
        ...
        if(collisionResolveMethod != -1 and collisionResolveSelectAlgorithm != -1):
            simulationParameters = (simulatorAmountOfRuns, schedulingAlgorithms.__getitem__(typeofSchedulingAlgorithm),attackTypes.__getitem__(typeOfSecurityAttack), intensivityOfTheAttack,attackStartTime,resolveMethods.__getitem__(collisionResolveMethod),selectAlgorithms.__getitem__(collisionResolveSelectAlgorithm))
        resultsStatistics = saveResults.writeDeleteResultsToFile(listOfResults, simulationParameters, now)
    if(returnedValue != None):
        returnedValue.append((schedulingAlgorithms.__getitem__(typeofSchedulingAlgorithm),resultsStatistics))
    return resultsStatistics

def runSimulator(typeOfSecurityAttack, intensivityOfTheAttack, attackStartTime, typeofSchedulingAlgorithm,collisionResolveMethod,collisionResolveSelectAlgorithm,simulatorAmountOfRuns,currentTime):
    ...
    selectAlgorithms = ['basedOnNBOfCollisions', 'basedOnTheSoonestDeadline', 'basedOnTheShortestDelay','basedOnTheEarliestArrivalTime']
    multiSchedulesFlag = False
    multiAttackStartTimesFlag = False
    multiAttackRatesFlag = False
    returnedValue = None
    returnedList = []
    now = currentTime
    ...
    try:
        if(("all" != typeofSchedulingAlgorithm.lower().replace(" ","")) and (int(typeofSchedulingAlgorithm)) not in range(len(schedulingAlgorithms))):
            raise Exception()
        elif ("all" != typeofSchedulingAlgorithm.lower().replace(" ","")):
            typeofSchedulingAlgorithm = int(typeofSchedulingAlgorithm)
    except:
        errorMessage = "Enter a correct value for the type of scheduling algorithm from 0 to 4 or 'all', where:\n" \
                       "\tall -->   to select all scheduling algorithm\n\t0   -->   SWTS\n\t1   -->   SWOTS_ASAP \n\t" \
                       "2   -->   SWOTS_ASAP_WS\n\t3   -->   SWOTS_AEAP\n\t4   -->   SWOTS_AEAP_WS"
        raise ValueError(errorMessage)
    try:
        if(("," not in attackStartTime) and ((float(attackStartTime))>1 or (float(attackStartTime))<0) ):
            raise Exception()
        elif("," not in attackStartTime):
            attackStartTime = float(attackStartTime)
    except:
        errorMessage = "Enter a correct value for the attack start time parameter from 0 to 1 or in the form 'startVelue,endValue,incrementValue' to select multiple values starting from startVelue to endValue with an increment of incrementValue"
        raise ValueError(errorMessage)
    try:
        if(("," not in intensivityOfTheAttack) and ((float(intensivityOfTheAttack))>1 or (float(intensivityOfTheAttack))<0) ):
            raise Exception()
        elif("," not in intensivityOfTheAttack):
            intensivityOfTheAttack = float(intensivityOfTheAttack)
    except:
        errorMessage = "Enter a correct value for the intensivity of the attack parameter from 0 to 1 or in the form 'startVelue,endValue,incrementValue' to select multiple values starting from startVelue to endValue with an increment of incrementValue"
        raise ValueError(errorMessage)
    try:
        simulatorAmountOfRuns = int(simulatorAmountOfRuns)
        if(simulatorAmountOfRuns <=0):
            raise Exception()
    except:
    ...
    if(typeofSchedulingAlgorithm == 'all'):
        multiSchedulesFlag = True
        if (not isinstance(attackStartTime, float) and "," in attackStartTime):
            multiAttackStartTimesFlag = True
            if (not isinstance(intensivityOfTheAttack, float) and "," in intensivityOfTheAttack):
                multiAttackRatesFlag = True
                # processes = []
                # for i in range(5):
                #     p = multiprocessing.Process(target=simulateCase3, args=(typeOfSecurityAttack, intensivityOfTheAttack, attackStartTime, i,simulatorAmountOfRuns, schedulingAlgorithms, attackTypes, now,))
                #     processes.append(p)
                #     p.start()
                #
                # for process in processes:
                ...
                for i in range(5):
                    if ((typeOfSecurityAttack in [2,3]) and ((collisionResolveMethod == 1 and i == 0) or ((collisionResolveSelectAlgorithm in [1,2,3]) and i ==0))):
                        if (collisionResolveMethod == 1 and i == 0):
                            print(R + "WARNING: The selected resolve method cannot be used with SWTS due to the schedule nature\nOnly resolving the collisionn by dropping the flow with highest number of collisions algorithm works with this scheduling algorithm.\n" + W)
                        elif (collisionResolveSelectAlgorithm in [1, 2, 3] and i == 0):
                            print(R + "WARNING: The selected algorithm cannot be used with SWTS due to the schedule nature\nOnly selecting the flow with highest number of collisions works with this scheduling algorithm.\n" + W)
                        continue
                    else:
                        returnedValue = simulateCase3(typeOfSecurityAttack, intensivityOfTheAttack, attackStartTime, i,simulatorAmountOfRuns, schedulingAlgorithms, attackTypes,resolveMethods,selectAlgorithms,collisionResolveMethod,collisionResolveSelectAlgorithm, now)
                        returnedList.append((schedulingAlgorithms.__getitem__(i),returnedValue))

            else:
                processes = []
                manager = multiprocessing.Manager()
                returnedList = manager.list()
                for i in range(5):
                    if ((typeOfSecurityAttack in [2,3]) and ((collisionResolveMethod == 1 and i == 0) or ((collisionResolveSelectAlgorithm in [1,2,3]) and i ==0))):
                        if (collisionResolveMethod == 1 and i == 0):
                            print(R + "WARNING: The selected resolve method cannot be used with SWTS due to the schedule nature\nOnly resolving the collisionn by dropping the flow with highest number of collisions algorithm works with this scheduling algorithm.\n" + W)
                        ...
                        p = multiprocessing.Process(target=simulateCase2, args=(typeOfSecurityAttack, intensivityOfTheAttack, attackStartTime, i, simulatorAmountOfRuns, schedulingAlgorithms, attackTypes,resolveMethods,selectAlgorithms,collisionResolveMethod,collisionResolveSelectAlgorithm, now,returnedList,))
                        processes.append(p)
                        p.start()
            ...
            if (not isinstance(intensivityOfTheAttack, float) and "," in intensivityOfTheAttack):
                ...
                for i in range(5):
                    if ((typeOfSecurityAttack in [2,3]) and ((collisionResolveMethod == 1 and i == 0) or ((collisionResolveSelectAlgorithm in [1,2,3]) and i ==0))):
                        ...
                        p = multiprocessing.Process(target=simulateCase1, args=(typeOfSecurityAttack, intensivityOfTheAttack, attackStartTime, i, simulatorAmountOfRuns, schedulingAlgorithms, attackTypes,resolveMethods,selectAlgorithms,collisionResolveMethod,collisionResolveSelectAlgorithm, now,returnedList,))
                        processes.append(p)
                        p.start()
                ...
                # for i in range(5):
                #     print(P + "{}".format(schedulingAlgorithms.__getitem__(i))+W)
                #     simulateCase1(typeOfSecurityAttack, intensivityOfTheAttack, attackStartTime, i, simulatorAmountOfRuns, schedulingAlgorithms, attackTypes, now)
        ...
        if ((typeOfSecurityAttack in [2, 3]) and ((collisionResolveMethod == 1 and typeofSchedulingAlgorithm == 0) or ((collisionResolveSelectAlgorithm in [1, 2, 3]) and typeofSchedulingAlgorithm == 0))):
            if (collisionResolveMethod == 1 and typeofSchedulingAlgorithm == 0):
                print(R + "WARNING: The selected resolve method cannot be used with SWTS due to the schedule nature\nOnly resolving the collisionn by dropping the flow with highest number of collisions algorithm works with this scheduling algorithm.\n" + W)
            elif (collisionResolveSelectAlgorithm in [1, 2, 3] and typeofSchedulingAlgorithm == 0):
                print(R + "WARNING: The selected algorithm cannot be used with SWTS due to the schedule nature\nOnly selecting the flow with highest number of collisions works with this scheduling algorithm.\n" + W)
            pass
            ...
            if(not isinstance(attackStartTime, float) and "," in attackStartTime):
                ...
                if (not isinstance(intensivityOfTheAttack, float) and "," in intensivityOfTheAttack):
                    multiAttackRatesFlag = True
                    returnedValue = simulateCase3(typeOfSecurityAttack, intensivityOfTheAttack, attackStartTime, typeofSchedulingAlgorithm,simulatorAmountOfRuns, schedulingAlgorithms, attackTypes,resolveMethods,selectAlgorithms,collisionResolveMethod,collisionResolveSelectAlgorithm, now)
                else:
                    returnedValue = simulateCase2(typeOfSecurityAttack, intensivityOfTheAttack, attackStartTime, typeofSchedulingAlgorithm,simulatorAmountOfRuns, schedulingAlgorithms, attackTypes,resolveMethods,selectAlgorithms,collisionResolveMethod,collisionResolveSelectAlgorithm, now)
            else:
                if (not isinstance(intensivityOfTheAttack, float) and "," in intensivityOfTheAttack):
                    multiAttackRatesFlag = True
                    returnedValue = simulateCase2(typeOfSecurityAttack, intensivityOfTheAttack, attackStartTime, typeofSchedulingAlgorithm,simulatorAmountOfRuns, schedulingAlgorithms, attackTypes,resolveMethods,selectAlgorithms,collisionResolveMethod,collisionResolveSelectAlgorithm, now)
                else:
                    returnedValue = simulateCase1(typeOfSecurityAttack,intensivityOfTheAttack,attackStartTime,typeofSchedulingAlgorithm,simulatorAmountOfRuns,schedulingAlgorithms,attackTypes,resolveMethods,selectAlgorithms,collisionResolveMethod,collisionResolveSelectAlgorithm,now)

            returnedList.append((schedulingAlgorithms.__getitem__(int(typeofSchedulingAlgorithm)),returnedValue))

    caseInfo = (multiSchedulesFlag,multiAttackStartTimesFlag,multiAttackRatesFlag)
    return (caseInfo,returnedList)


def startSimulate(typeOfSecurityAttack, intensivityOfTheAttack, attackStartTime, typeofSchedulingAlgorithm,collisionResolveMethod, collisionResolveSelectAlgorithm, simulatorAmountOfRuns):
    currentTime = datetime.now()
    if(typeOfSecurityAttack in [0,1]):
        runSimulator(typeOfSecurityAttack, intensivityOfTheAttack, attackStartTime, typeofSchedulingAlgorithm, -1, -1,simulatorAmountOfRuns,currentTime)
        ...
        resolveMethods = ['collisionResolveByDropping', 'collisionResolveByDelaying']
        selectAlgorithms = ['basedOnNBOfCollisions', 'basedOnTheSoonestDeadline', 'basedOnTheShortestDelay','basedOnTheEarliestArrivalTime']
        multiCollisionResolveMethodFlag = False
        multiCollisionResolveSelectAlgorithmFlag = False
        listOfSchedules = []
        listOfAttackStartTimes = []
        listOfAttackRates = []
        listOfResolveMethods = []
        listOfResolveSelectAlgorithms = []
        listOfMins = []
        listOfMaxs = []
        listOfMeans = []
        listOfMeans1 = []
        ...
        if (collisionResolveMethod == 'all' and collisionResolveSelectAlgorithm == 'all'):
            multiCollisionResolveMethodFlag = True
            multiCollisionResolveSelectAlgorithmFlag = True
            for i in range(2):
                print(P + "\t# Collision Resolve Method: {}".format(resolveMethods.__getitem__(i)) + W)
                for j in range(4):
                    print(C + "\t\t# Collision Resolve Select Algorithm: {}".format(selectAlgorithms.__getitem__(j)) + W)
                    caseInformation, resultsSummary = runSimulator(typeOfSecurityAttack, intensivityOfTheAttack, attackStartTime, typeofSchedulingAlgorithm,i, j, simulatorAmountOfRuns,currentTime)
                    for tempSchedulingAlgorithm , tempResult in resultsSummary:
                        if(caseInformation.__getitem__(1) or caseInformation.__getitem__(2)):
                            listOfSchedules.extend([tempSchedulingAlgorithm for x in range(len(tempResult.__getitem__(0)))])
                            listOfResolveMethods.extend([resolveMethods.__getitem__(i) for x in range(len(tempResult.__getitem__(0)))])
                            listOfResolveSelectAlgorithms.extend([selectAlgorithms.__getitem__(j) for x in range(len(tempResult.__getitem__(0)))])
                            if(caseInformation.__getitem__(1) and caseInformation.__getitem__(2)):
                                listOfAttackStartTimes.extend(tempResult.__getitem__(0))
                                listOfAttackRates.extend(tempResult.__getitem__(1))
                                listOfMins.extend(tempResult.__getitem__(2))
                                listOfMaxs.extend(tempResult.__getitem__(3))
                                listOfMeans.extend(tempResult.__getitem__(4))
                                listOfMeans1.extend(tempResult.__getitem__(5))
                                listOfMeans2.extend(tempResult.__getitem__(6))
                                listOfMeans3.extend(tempResult.__getitem__(7))
                                listOfMedians.extend(tempResult.__getitem__(8))
                                listOfModes.extend(tempResult.__getitem__(9))
                                listOfStDevs.extend(tempResult.__getitem__(10))
                                listOfVariances.extend(tempResult.__getitem__(11))
                            else:
                                listOfAttackStartTimes.extend(tempResult.__getitem__(0)) if(caseInformation.__getitem__(1)) else listOfAttackStartTimes.extend([attackStartTime for x in range(len(tempResult.__getitem__(0)))])
                                listOfAttackRates.extend(tempResult.__getitem__(0)) if(caseInformation.__getitem__(2)) else listOfAttackRates.extend([intensivityOfTheAttack for x in range(len(tempResult.__getitem__(0)))])
                                listOfMins.extend(tempResult.__getitem__(1))
                                listOfMaxs.extend(tempResult.__getitem__(2))
                                listOfMeans.extend(tempResult.__getitem__(3))
                                listOfMeans1.extend(tempResult.__getitem__(4))
                                listOfMeans2.extend(tempResult.__getitem__(5))
                                listOfMeans3.extend(tempResult.__getitem__(6))
                                listOfMedians.extend(tempResult.__getitem__(7))
                                listOfModes.extend(tempResult.__getitem__(8))
                                listOfStDevs.extend(tempResult.__getitem__(9))
                                listOfVariances.extend(tempResult.__getitem__(10))


                        else:
                            listOfSchedules.append(tempSchedulingAlgorithm)
                            listOfAttackStartTimes.append(attackStartTime)
                            listOfAttackRates.append(intensivityOfTheAttack)
                            listOfResolveMethods.append(resolveMethods.__getitem__(i))
                            listOfResolveSelectAlgorithms.append(selectAlgorithms.__getitem__(j))
                            listOfMins.append(tempResult.__getitem__(0))
                            listOfMaxs.append(tempResult.__getitem__(1))
                            listOfMeans.append(tempResult.__getitem__(2))
                            listOfMeans1.append(tempResult.__getitem__(3))
                            listOfMeans2.append(tempResult.__getitem__(4))
                            listOfMeans3.append(tempResult.__getitem__(5))
                            listOfMedians.append(tempResult.__getitem__(6))
                            listOfModes.append(tempResult.__getitem__(7))
                            listOfStDevs.append(tempResult.__getitem__(8))
                            listOfVariances.append(tempResult.__getitem__(9))




        elif (collisionResolveMethod == 'all' and collisionResolveSelectAlgorithm != 'all'):
            multiCollisionResolveMethodFlag = True
            for i in range(2):
                print(P + "\t# Collision Resolve Method: {}".format(resolveMethods.__getitem__(i)) + W)
                print(C + "\t\t# Collision Resolve Select Algorithm: {}".format(selectAlgorithms.__getitem__(collisionResolveSelectAlgorithm)) + W)
                caseInformation, resultsSummary = runSimulator(typeOfSecurityAttack, intensivityOfTheAttack, attackStartTime, typeofSchedulingAlgorithm, i,collisionResolveSelectAlgorithm, simulatorAmountOfRuns,currentTime)
                for tempSchedulingAlgorithm, tempResult in resultsSummary:
                    if (caseInformation.__getitem__(1) or caseInformation.__getitem__(2)):
                        listOfSchedules.extend([tempSchedulingAlgorithm for x in range(len(tempResult.__getitem__(0)))])
                        listOfResolveMethods.extend([resolveMethods.__getitem__(i) for x in range(len(tempResult.__getitem__(0)))])
                        listOfResolveSelectAlgorithms.extend([selectAlgorithms.__getitem__(collisionResolveSelectAlgorithm) for x in range(len(tempResult.__getitem__(0)))])
                        if (caseInformation.__getitem__(1) and caseInformation.__getitem__(2)):
                            listOfAttackStartTimes.extend(tempResult.__getitem__(0))
                            listOfAttackRates.extend(tempResult.__getitem__(1))
                            listOfMins.extend(tempResult.__getitem__(2))
                        ...
                        listOfAttackRates.append(intensivityOfTheAttack)
                        listOfResolveMethods.append(resolveMethods.__getitem__(i))
                        listOfResolveSelectAlgorithms.append(selectAlgorithms.__getitem__(collisionResolveSelectAlgorithm))
                        listOfMins.append(tempResult.__getitem__(0))
                        listOfMaxs.append(tempResult.__getitem__(1))
                        listOfMeans.append(tempResult.__getitem__(2))
                        listOfMeans1.append(tempResult.__getitem__(3))
            ...
            multiCollisionResolveSelectAlgorithmFlag = True
            print(P + "\t# Collision Resolve Method: {}".format(resolveMethods.__getitem__(collisionResolveMethod)) + W)
            for j in range(4):
                print(C + "\t\t# Collision Resolve Select Algorithm: {}".format(selectAlgorithms.__getitem__(j)) + W)
                caseInformation, resultsSummary = runSimulator(typeOfSecurityAttack, intensivityOfTheAttack, attackStartTime, typeofSchedulingAlgorithm,collisionResolveMethod, j, simulatorAmountOfRuns,currentTime)
                for tempSchedulingAlgorithm, tempResult in resultsSummary:
                    if (caseInformation.__getitem__(1) or caseInformation.__getitem__(2)):
                        listOfSchedules.extend([tempSchedulingAlgorithm for x in range(len(tempResult.__getitem__(0)))])
                        listOfResolveMethods.extend([resolveMethods.__getitem__(collisionResolveMethod) for x in range(len(tempResult.__getitem__(0)))])
                        listOfResolveSelectAlgorithms.extend([selectAlgorithms.__getitem__(j) for x in range(len(tempResult.__getitem__(0)))])
                        if (caseInformation.__getitem__(1) and caseInformation.__getitem__(2)):
                            listOfAttackStartTimes.extend(tempResult.__getitem__(0))
                            listOfAttackRates.extend(tempResult.__getitem__(1))
                        ...
                        listOfAttackStartTimes.append(attackStartTime)
                        listOfAttackRates.append(intensivityOfTheAttack)
                        listOfResolveMethods.append(resolveMethods.__getitem__(collisionResolveMethod))
                        listOfResolveSelectAlgorithms.append(selectAlgorithms.__getitem__(j))
                        listOfMins.append(tempResult.__getitem__(0))
                        listOfMaxs.append(tempResult.__getitem__(1))
                        listOfMeans.append(tempResult.__getitem__(2))
            ...
            print(P + "\t# Collision Resolve Method: {}".format(resolveMethods.__getitem__(collisionResolveMethod)) + W)
            print(C + "\t\t# Collision Resolve Select Algorithm: {}".format(selectAlgorithms.__getitem__(collisionResolveSelectAlgorithm)) + W)
            caseInformation, resultsSummary = runSimulator(typeOfSecurityAttack, intensivityOfTheAttack, attackStartTime, typeofSchedulingAlgorithm,collisionResolveMethod, collisionResolveSelectAlgorithm, simulatorAmountOfRuns,currentTime)
            for tempSchedulingAlgorithm, tempResult in resultsSummary:
                if (caseInformation.__getitem__(1) or caseInformation.__getitem__(2)):
                    listOfSchedules.extend([tempSchedulingAlgorithm for x in range(len(tempResult.__getitem__(0)))])
                    listOfResolveMethods.extend([resolveMethods.__getitem__(collisionResolveMethod) for x in range(len(tempResult.__getitem__(0)))])
        ...
        simulationParameters = (simulatorAmountOfRuns, "all" if caseInformation.__getitem__(0) else schedulingAlgorithms.__getitem__(int(typeofSchedulingAlgorithm)),attackTypes.__getitem__(typeOfSecurityAttack), intensivityOfTheAttack, attackStartTime, 'all' if multiCollisionResolveMethodFlag else resolveMethods.__getitem__(collisionResolveMethod),'all' if multiCollisionResolveSelectAlgorithmFlag else selectAlgorithms.__getitem__(collisionResolveSelectAlgorithm))
        newCaseInformation = (caseInformation.__getitem__(0),caseInformation.__getitem__(1),caseInformation.__getitem__(2),multiCollisionResolveMethodFlag,multiCollisionResolveSelectAlgorithmFlag)
        tempList = []
        for i in range(len(listOfMeans3)):
            tempList.append((listOfSchedules.__getitem__(i), listOfAttackStartTimes.__getitem__(i),listOfAttackRates.__getitem__(i), listOfResolveMethods.__getitem__(i),listOfResolveSelectAlgorithms.__getitem__(i), listOfMins.__getitem__(i),listOfMaxs.__getitem__(i), listOfMeans.__getitem__(i), listOfMeans1.__getitem__(i),listOfMeans2.__getitem__(i), listOfMeans3.__getitem__(i), listOfMedians.__getitem__(i),listOfModes.__getitem__(i), listOfStDevs.__getitem__(i), listOfVariances.__getitem__(i)))
        orderedListOfResults = sorted(tempList, key=lambda x: (x.__getitem__(0), x.__getitem__(1), x.__getitem__(2), x.__getitem__(3), x.__getitem__(4)), reverse=False)
        newSummaryOfResults = (newCaseInformation, orderedListOfResults)
        saveResults.writeDeleteResultsToFileCase3(newSummaryOfResults,simulationParameters,currentTime)
...
def main():
    ...
    intensivityOfTheAttack = '0,1,0.05'      # How strong is the attack (where 0 is none and 1 is all)
    ...
    attackStartTime = '0,1,0.05'               # when the attack will start (0   = at the beginning
                                        #                             1   = at the end
    ...
    typeofSchedulingAlgorithm = 'all'     # The used scheduling algorithm (0 = SWTS
                                        #                                1 = SWOTS_ASAP
                                        #                                2 = SWOTS_ASAP_WS
                                        #                                3 = SWOTS_AEAP
                                        #                                4 = SWOTS_AEAP_WS
    ...
    simulatorAmountOfRuns = '10'        # How many times the simulator will run (from 1 to infinite). E.g., 2 will make the Simulator run twice. If we choose 'all' in 'typeofSchedulingAlgorithm',
    ...
    collisionResolveMethod = 'all'         # 0 for dropping, 1 for delaying, -1 for none, 'all' for all

    collisionResolveSelectAlgorithm= 'all'  # -1,0,1,2,3, or 'all' check the method body to understand the meaning of this algorithms
    ...
    # Simulator Run Start
    # start = timer()
    # if(((collisionResolveSelectAlgorithm not in [0,1,2,3]) and (not isinstance(collisionResolveSelectAlgorithm,str) or (isinstance(collisionResolveSelectAlgorithm,str) and collisionResolveSelectAlgorithm.lower() != 'all'))) or ((collisionResolveMethod not in [0,1]) and (not isinstance(collisionResolveMethod,str) or (isinstance(collisionResolveMethod,str) and collisionResolveMethod.lower() != 'all')))):
    #     currentTime = datetime.now()
    #     runSimulator(typeOfSecurityAttack, intensivityOfTheAttack, attackStartTime, typeofSchedulingAlgorithm,collisionResolveMethod, collisionResolveSelectAlgorithm, simulatorAmountOfRuns,currentTime)
    # else:
    #     startSimulate(typeOfSecurityAttack, intensivityOfTheAttack, attackStartTime, typeofSchedulingAlgorithm,collisionResolveMethod, collisionResolveSelectAlgorithm, simulatorAmountOfRuns)
    # end = timer()
    # runningTime = round(((end - start) / 60), 2)
    # print(P + "The running time is {} minutes".format(runningTime) + W)
    # Simulator Run End
